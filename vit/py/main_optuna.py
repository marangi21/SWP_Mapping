...
def objective(trial, batch_size, n_epochs, device, seed):
    ...
    try:
        ...
        joblib.dump(study, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results.pkl'))
        raise
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results.pkl')):
        study = optuna.create_study(direction=StudyDirection.MINIMIZE, sampler=sampler)
    else:
        study = joblib.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results.pkl'))
