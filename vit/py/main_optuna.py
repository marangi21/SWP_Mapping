...
def objective(trial, batch_size, n_epochs, device, seed):
    ...
    try:
        ...
        train_set = TreeDataset.TreeDataset(mode='train', augment = False)
        ...
        joblib.dump(study, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_noaug.pkl'))
        raise
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_noaug.pkl')):
        study = optuna.create_study(direction=StudyDirection.MINIMIZE, sampler=sampler)
    else:
        study = joblib.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_noaug.pkl'))
