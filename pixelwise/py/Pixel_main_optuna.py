...
from pixels_utilities import train_model, plot_best_losses
import os
import joblib
import argparse
...
def objective(trial, batch_size, n_epochs, device, seed):
    ...
    try:
        ...
        #creazione modello
        model = PixelNet.PixelNet(train_set.X.shape[1], num_hidden_layers, size_hidden_layers, dropout_rates, device)
        ...
        joblib.dump(study, os.path.join(os.path.abspath(__file__), '..', 'optuna_results_pixelwise.pkl'))
        raise
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_pixelwise.pkl')):
        study = optuna.create_study(direction=StudyDirection.MINIMIZE, sampler=sampler)
    else:
        study = joblib.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_pixelwise.pkl'))
    ...
    with tqdm(total=n_trials, desc="Optimizing.. ") as trial_pbar:
        def callback(study, trial):
            ...
            trial_pbar.update(1)
            joblib.dump(study, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'optuna_results_pixelwise.pkl'))
